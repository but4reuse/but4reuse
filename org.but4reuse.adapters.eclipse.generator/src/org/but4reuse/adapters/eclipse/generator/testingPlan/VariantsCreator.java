...
/**
 * This class generate a lot of variants (following the testing plan in
 * src/resources/TestingPlan.txt), in specific directories. Each line of this
 * file lead a creation of 3 variants, with the inputs and random specified. <br>
...
public class VariantsCreator {
	...
	public void process() {
		...
		String input;
		if (VariantsUtils.isEclipseDir(inputDir))
			input = inputDir.getParent();
		else
			input = inputDir.getPath();
		...
		if (!testingPlanFile.exists()) {
			System.out.println("\nsrc" + File.separator + "resources" + File.separator + TESTING_PLAN
					+ " not exists...");
			scan.close();
			return;
		}
		...
		for (String existingInput : allExistingInputs) {
			try {
				...
				if (!output_tmp.endsWith(File.separator))
					output_tmp += File.separator;
				new VariantsGenerator(input + existingInput, output_tmp + existingInput + "-" + choice, NB_VARIANTS,
						Integer.parseInt(choice)).generate();
			} catch (Exception e) {
				System.out.println("Eror with the creation of " + existingInput + " : " + e);
				e.printStackTrace();
			}
			System.out.println("\n===================================================\n");
		}
	...
	private List<String> getAllChoices(BufferedReader testingPlan) throws IOException {
		if (testingPlan == null || !testingPlan.ready())
			return null;
	...
	private List<String> getAllInputsFrom(BufferedReader testingPlan) throws IOException {
		...
		String line = null; // ex: JEE_Testing or #Variants with 10%
		while ((line = testingPlan.readLine()) != null) {
