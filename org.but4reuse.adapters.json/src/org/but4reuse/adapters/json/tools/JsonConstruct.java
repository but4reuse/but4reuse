...
public class JsonConstruct {
	...
	private JsonValue construct(JsonObject root, IElement element, JsonValue value) {
		...
		if (element instanceof IgnoredElement) {
			...
			return construct(root, ignoredElement.parent, ignoredElement.jsonValue);
		}
		if (element instanceof IndexArrayElement) {
			IndexArrayElement indexArrayElement = (IndexArrayElement) element;
			JsonArray jsonArray = construct(root, indexArrayElement.parent, null).asArray();
		...
		if (element instanceof KeyElement) {
			...
			if (keyElement.parent == null)
				...
				jsonObject = construct(root, keyElement.parent, null).asObject();
		...
		if (element instanceof UnsplittableElement) {
			...
			return construct(root, unsplittableElement.parent, unsplittableElement.content);
		}
		if (element instanceof ValueElement) {
			ValueElement valueElement = (ValueElement) element;
			return this.construct(root, valueElement.parent, valueElement.jsonValue);
		}
		return value;
	}
}
