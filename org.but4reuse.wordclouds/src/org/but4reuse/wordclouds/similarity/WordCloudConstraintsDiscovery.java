...
public class WordCloudConstraintsDiscovery implements IConstraintsDiscovery {
	...
	public List<IConstraint> discover(FeatureList featureList, AdaptedModel adaptedModel, Object extra,
		...
		for(int i = 0; i< nb_Block;i++)
		{
			...
			for(int j = 0;j<nb_Block;j++)
			{
				...
				/*
				 * We check if c1 is close to c2 (similarity) and if each time we have b1 in an artefact there is b2 in it too.
				 */
				double similarity = WordCloudUtil.cmpClouds(cloud_b1,cloud_b2 );
				if(inSameArtefact(adaptedModel.getOwnedAdaptedArtefacts(), b1, b2) 
						&& similarity > rateMin)
				{
	 ...
	 * @param b1 The first block
	 * @param b2 The second block
	 * @param similarity The similarity between b1 and b2
