...
public class DoubleFieldEditor extends FieldEditor {
	...
	/**
	 * Constructor
	 * @param valueName Preference name
	 * @param name label name
	 * @param parent parent composite
	 */
	public DoubleFieldEditor(String valueName,String name, Composite parent, Activator act)
	{
		super(valueName,name,parent);
		store = act.getPreferenceStore();
	}
	...
	protected void doFillIntoGrid(Composite parent, int numColumns) {
		...
		scale.addSelectionListener(new SelectionListener() {
			...
			public void widgetSelected(SelectionEvent e) {
				/*
				 * when the selection scale change we update the value in the text control
				 */
		...
		text.addVerifyListener(new VerifyListener() {
			...
			public void verifyText(VerifyEvent e) {
				...
				if((int)e.character == 8) //It means backspace char so we remove the last char
				{
				...
				else if(text.getSelectionCount() > 0) // If the user has selected some chars we remove if
				{
				...
				if((int)e.character != 0) // If the user add a char in the control 
				{
					...
					if(Character.isDigit(e.character) || e.character == '.')
					{
						...
						/*
						 * Here we check if the user has gave a number and if it's between 0 and 1 
						 */
						try {
				...
				else
				{
					try {
					/*
					 * Here it's for when we update the Text control with setText method
					 * we check if the number is between 0 and 1
					 */
	...
	protected void doLoad() {
		value = store.getDouble(getPreferenceName());
		scale.setSelection((int)(value*100));
		String st = String.format("%.2f", value);
		st =st.replace(",", ".");
		text.setText(st);
	...
	protected void doLoadDefault() {
		value = store.getDefaultDouble(getPreferenceName());
		scale.setSelection((int)(value*100));
		String st = String.format("%.2f", value);
		st =st.replace(",", ".");
		text.setText(st);
