...
public class JavaPrintVisitor extends ArtifactPrintVisitor {
	...
	public void processNode(FSTNode node, File folderPath) throws PrintVisitorException {
		if(node instanceof FSTNonTerminal) {
			...
			//System.out.println("CHILD of :"+nonterminal.getType()+ " "+nonterminal.getChildren().size());
			//for(FSTNode child : nonterminal.getChildren()) {
				...
				try {
					...
					//if (child instanceof FSTNonTerminal) visitor.visit((FSTNonTerminal)child);
					//if (child instanceof FSTTerminal) visitor.visit((FSTTerminal)child);
					visitor.getResult();
