...
public class FileAndDirectoryUtils {
	...
	public static File[] getAllJarsInDirectory(File dir) {
		if(dir==null || dir.isFile()) return null;
	...
	public static void copyDirectoryWithFilters(File dir, String output, FileFilter filter) throws IOException{
		try {
			...
			if(filter==null) FileUtils.copyDirectory(dir, newDir);
			else FileUtils.copyDirectory(dir, newDir, filter);
	 ...
	 * @param output : The destination
	 * @param files : All the files to copy
	 * @throws IOException if the copy is impossible
	 */
	public static void copyFilesAndDirectories(String output, File... files) throws IOException{
		try {
			for(File fic : files){
				if(fic.isDirectory()) copyDirectory(fic, output);
				else FileUtils.copyFileToDirectory(fic, new File(output));
			}
		} catch (IOException exc) {
	 ...
	 * @param outputFile : The files to copy
	 * @throws IOException if the copy is impossible
	 */
	...
	/**
	 * Return a random (based on the percentage parameter) list of File, from one liste.
	 * For example, getSomeFiles( [2,8,9,1,13], 50 ) can return [8, 1, 13].
	 * @param listFiles : The File list
	 * @param percentage : The random (between 0 and 100%)
	 * @return
	 */
	public static File[] getSomeFiles(File[] listFiles, int percentage) {
		if(listFiles!=null || percentage==100){
			...
			for(File file : listFiles){
				if(Math.random()*100 < percentage) res.add(file);
			}
