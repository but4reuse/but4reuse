	...
	public void generate() {
		...
		List<FileElement> allFileElements = new ArrayList<>();
		{
			...
			for(IElement elem : iElems){
				if(elem instanceof PluginElement) allPlugins.add((PluginElement) elem);
				else if(elem instanceof FileElement) allFileElements.add((FileElement) elem);
			}
		}
		sendToAll("Total features number in the input = "+allFeatures.size());
		sendToAll("Total plugins number in the input = "+allPlugins.size()+"\n");
		
		FeaturesAndPluginsDependencies depOperator = new FeaturesAndPluginsDependencies(allFeatures, (List<PluginElement>) allPlugins, inputURI.toString());
		...
		for(int i=1; i<=nbVariants ; i++){
			...
			if(percentage!=100 && percentage!=0){
				...
				for(ActualFeature chosenFeature : chosenFeatures){ // Get all plugins dependencies from chosen features
